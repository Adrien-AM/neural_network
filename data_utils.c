...
void read_columns(FILE *f, struct csv *csv)
{
...
struct csv *extract_target_from_data(char *target, struct csv *data)
{
    ...
    for (size_t i = 0; i < data->nb_lines; i++)
    {
        ...
        data->data[i][target_index] = data->data[i][data->nb_columns - 1]; // replace target by last col
        data->data[i] = realloc(data->data[i], sizeof(float) * data->nb_columns - 1);
    }

    // reallocate memory for column name, then rename
    data->columns[target_index] = realloc(data->columns[target_index], sizeof(char) * strlen(data->columns[data->nb_columns - 1]) + 1);
